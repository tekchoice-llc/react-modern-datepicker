...
class ModernDatepicker extends Component {
  ...
  handleDateChange(value, unit) {
    const { format, onChange, maxDate, minDate, defaultDateView } = this.props;
    ...
    let defaultDate = '';
    if (defaultDateView && !this.state.dateToEdit && dayjs(defaultDateView).isValid()) {
  ...
  render() {
    const { defaultDateView, disabled, sortHeader, allowEdit, format, placeholder, showBorder, className, id, icon, iconClass, maxDate, minDate, label, labelClass, lang, primaryColor = '#00b9f5', secondaryColor = 'white', primaryTextColor = 'black', secondaryTextColor = 'white' } = this.props;
